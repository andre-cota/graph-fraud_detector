import time
import pandas as pd
from algorithms.detecao_ciclos import encontrar_ciclos_fraude
from interface.svg_utils import desenhar_grafo_svg

def exibir_ciclos_fraude(grafo, st):
    inicio = time.time()
    ciclos = encontrar_ciclos_fraude(grafo, limite_tamanho=6, tamanho_min=3)
    tempo_exec = time.time() - inicio
    st.write(f"Total de ciclos fraudulentos encontrados: {len(ciclos)}")
